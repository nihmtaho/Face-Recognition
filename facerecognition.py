    ...
    while True:
        ...
        for (x, y, w, h) in face:
            ...
            if profile is not None:
                cv2.putText(frame, "ID: " + str(profile[0]), (x, y - 30), font, font_scale, fontcolor,
                            lineType=cv2.LINE_AA)
                cv2.putText(frame, "NAME: " + str(profile[1]), (x, y - 10), font, font_scale, fontcolor,
                            lineType=cv2.LINE_AA)
            else:
                cv2.putText(frame, "Opps!!!", (x, y - 10), font, font_scale, fontcolor, lineType=cv2.LINE_AA)
